...
namespace SVAAnalyzer
{
	 ...
	 * GPU build priority: TensorRT > CUDA. GPU builds fail fast instead of using CPU.
	 ...
	 * - CPU: Available only in an explicit SVA_ONNXRUNTIME_GPU=OFF build.
	 */
	...
	private:
		...
		bool decodePoseOutputWithNms(const float *pdata,
								 int imageWidth,
								 int imageHeight,
								 float scale,
								 float padX,
